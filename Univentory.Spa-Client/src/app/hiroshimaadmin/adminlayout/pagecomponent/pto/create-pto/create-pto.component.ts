...
/**
 * Create PTO component
 */
@Component({
    selector: 'create-pto-modal',
    templateUrl: './create-pto.component.html'
})

export class CreatePTOComponent extends LanguageHandlerService implements OnInit {
    /**
     * Create PTO modal
     */
    @ViewChild('createPTOModal') modal: ModalDirective;
   /**
  ...
  */
    ...
    */
    save(): void {
        ...
        // this.createNewPTO.IsActive = this.f.isActive.value;
        this.createNewPTO.PTODescription.push(this.addPTOForm.value);
        this.ptoService.CreatePTO(this.createNewPTO)
            .pipe(first())
            .subscribe(response => {
                if (!response.success)
